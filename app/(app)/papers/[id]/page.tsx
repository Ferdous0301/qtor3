...
import { Archive, ArrowDown, ArrowLeft, ArrowUp, Check, Copy, FileText, GripVertical, LockKeyhole, MoreHorizontal, Plus, Save, Trash2, Undo2 } from "lucide-react"
...
import { ExportDialog } from "@/components/papers/export-dialog"
...
export default function PaperEditorPage() {
  ...
  return <PageContainer>
    ...
    {previewQuestion ? <QuestionEditor open={Boolean(previewQuestion)} snapshot={previewQuestion.snapshot} onOpenChange={(open) => { if (!open) setPreviewQuestion(null) }} /> : null}{exportOpen ? <ExportDialog open={exportOpen} onOpenChange={setExportOpen} paperId={id} title={title} questions={questions} locked={locked} onExported={() => void loadPaper()} /> : null}{rewriteQuestion ? <RewriteDialog open={Boolean(rewriteQuestion)} onOpenChange={(open) => { if (!open) setRewriteQuestion(null) }} items={[{ id: rewriteQuestion.id, text: rewriteQuestion.snapshot.question_text, sourceQuestionId: rewriteQuestion.source_question_id ?? undefined, customContentId: rewriteQuestion.custom_content_id ?? undefined }]} /> : null}
  </PageContainer>
}
