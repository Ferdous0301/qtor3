...
const API_ERROR_COPY: Record<string, string> = { RATE_LIMITED: "Please try again in a moment.", PAPER_QUESTION_LIMIT_REACHED: "This paper has reached its question limit.", PAPER_NOT_EDITABLE: "This paper was exported and is locked.", TAXONOMY_NOT_FOUND: "That selection is no longer available. Please choose again.", SEARCH_WINDOW_TOO_DEEP: "Showing only the first 10,000 results. Refine your search to see more.", PAPER_HAS_NO_QUESTIONS: "Add at least one question first.", PAPER_EXCEEDS_MAXIMUM_TIER: "This paper is too large for the available export tiers.", INVALID_IDEMPOTENCY_KEY: "Please try that action again.", ENTITLEMENT_ALREADY_USED: "That entitlement has already been used.", PURCHASE_NOT_CONFIRMED: "The purchase is not confirmed yet.", TEMPLATE_EXTRACTION_FAILED: "The template could not be read. You were not charged.", JOB_FAILED: "The background job failed. You were not charged.", EXPORT_FAILED: "The export failed. You were not charged.", VALIDATION_ERROR: "Please check the highlighted fields.", UNAUTHORIZED: "Your session has expired. Please sign in again.", FORBIDDEN: "You do not have permission to do that." }
export function errorMessage(error: unknown) { if (error instanceof ApiError) { const copy = API_ERROR_COPY[error.code] ?? "Something went wrong. Please try again."; return error.code === "RATE_LIMITED" ? `${copy} Try again in ${Number(error.details.retry_after_seconds) || 1} seconds.` : copy } return error instanceof Error ? error.message : "Something went wrong. Please try again." }
