...
function QuestionCard({ item, index, count, locked, onMove, onRemove, onSection, onDrop, onPreview }: { item: PaperQuestion; index: number; count: number; locked: boolean; onMove: (direction: -1 | 1) => void; onRemove: () => void; onSection: (value: string) => void; onDrop: (draggedId: string) => void; onPreview: () => void }) {
  ...
  return <article draggable={!locked} onDragStart={(event) => event.dataTransfer.setData("text/plain", item.id)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); onDrop(event.dataTransfer.getData("text/plain")) }} className="group flex gap-3 border-b px-4 py-5 transition-colors last:border-0 hover:bg-muted/20 sm:px-6">
    ...
    <div className="flex shrink-0 flex-col gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100"><Button variant="ghost" size="sm" onClick={onPreview}>View</Button><Button variant="ghost" size="icon-xs" disabled={locked || index === 0} onClick={() => onMove(-1)} aria-label={`Move question ${index + 1} up`}><ArrowUp /></Button><Button variant="ghost" size="icon-xs" disabled={locked || index === count - 1} onClick={() => onMove(1)} aria-label={`Move question ${index + 1} down`}><ArrowDown /></Button><Button variant="ghost" size="icon-xs" disabled={locked} onClick={onRemove} aria-label={`Remove question ${index + 1}`}><Trash2 /></Button></div>
  </article>
}
...
export default function PaperEditorPage() {
  ...
  return <PageContainer>
    ...
    {previewQuestion ? <QuestionEditor open={Boolean(previewQuestion)} snapshot={previewQuestion.snapshot} onOpenChange={(open) => { if (!open) setPreviewQuestion(null) }} /> : null}
  </PageContainer>
}
